...
use crate::models::{
    ApiResponse, CzceWarehouseReceiptResponse, DceWarehouseReceipt, ForeignFuturesDetail,
    ForeignFuturesHistData, Futures99Symbol, FuturesCommInfo, FuturesCommQuery,
    FuturesContractDetail, FuturesFeesInfo, FuturesHistoryData, FuturesHoldPosQuery,
    FuturesHoldPosition, FuturesInfo, FuturesInventory99, FuturesInventory99Query,
    FuturesMainContract, FuturesMainDailyData, FuturesMainQuery, FuturesQuery, FuturesRule,
    FuturesRuleQuery, FuturesSpotPrice, FuturesSpotPriceDailyQuery, FuturesSpotPricePrevious,
    FuturesSpotPricePreviousQuery, FuturesSpotPriceQuery, FuturesSymbolMark,
    GfexWarehouseReceiptResponse, RankSum, RankSumDailyQuery, RankTableQuery, RankTableResponse,
...
use crate::services::futures::{
    ...
    futures_warehouse_receipt_dce, get_99_symbol_map, get_cffex_rank_table, get_dce_rank_table,
    get_foreign_futures_realtime, get_foreign_futures_symbols, get_futures_comm_info,
    get_futures_display_main_sina, get_futures_fees_info, get_futures_foreign_detail,
    get_futures_foreign_hist, get_futures_history, get_futures_hold_pos_sina,
    get_futures_inventory_99, get_futures_main_sina, get_futures_minute_data, get_futures_rule,
    get_futures_spot_price, get_futures_spot_price_daily, get_futures_spot_price_previous,
    get_gfex_rank_table, get_rank_sum, get_rank_sum_daily, get_rank_table_czce,
    get_shfe_rank_table, FuturesService,
};
...
#[cfg_attr(feature = "swagger", utoipa::path(
    ...
    tag = "futures",
    params(
        ("symbol" = String, Path, description = "合约代码（如 RB2510）")
    ),
    responses(
        (status = 200, description = "成功获取期货合约信息", body = ApiResponse<FuturesInfo>),
...
#[cfg_attr(feature = "swagger", utoipa::path(
    get,
    path = "/futures/rank/shfe",
    tag = "futures-rank",
    params(RankTableQuery),
    responses(
        (status = 200, description = "成功获取持仓排名数据", body = ApiResponse<Vec<RankTableResponse>>),
        (status = 500, description = "服务器错误")
    )
))]
pub async fn get_rank_shfe(query: web::Query<RankTableQuery>) -> Result<HttpResponse> {
    let vars = query.vars.as_ref().map(|v| {
        v.split(',')
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .collect()
    ...
    match get_shfe_rank_table(&query.date, vars).await {
        ...
        Err(e) => Ok(HttpResponse::InternalServerError()
            .json(ApiResponse::<Vec<RankTableResponse>>::error(e.to_string()))),
    }
}
...
#[cfg_attr(feature = "swagger", utoipa::path(
    get,
    path = "/futures/rank/sum",
    tag = "futures-rank",
    params(RankTableQuery),
    responses(
        (status = 200, description = "成功获取持仓排名汇总", body = ApiResponse<Vec<RankSum>>),
        (status = 500, description = "服务器错误")
    )
))]
pub async fn get_rank_sum_data(query: web::Query<RankTableQuery>) -> Result<HttpResponse> {
    ...
    match get_rank_sum(&query.date, vars).await {
        Ok(data) => Ok(HttpResponse::Ok().json(ApiResponse::success(data))),
        Err(e) => Ok(HttpResponse::InternalServerError()
            .json(ApiResponse::<Vec<RankSum>>::error(e.to_string()))),
    }
}
...
#[cfg_attr(feature = "swagger", utoipa::path(
    get,
    path = "/futures/rank/sum_daily",
    tag = "futures-rank",
    params(RankSumDailyQuery),
    responses(
        (status = 200, description = "成功获取持仓排名汇总（日期区间）", body = ApiResponse<Vec<RankSum>>),
        (status = 500, description = "服务器错误")
    )
))]
...
#[cfg_attr(feature = "swagger", utoipa::path(
    get,
    path = "/futures/warehouse/czce",
    tag = "futures-warehouse",
    params(RankTableQuery),
    responses(
        (status = 200, description = "成功获取仓单日报", body = ApiResponse<Vec<CzceWarehouseReceiptResponse>>),
        (status = 500, description = "服务器错误")
    )
))]
pub async fn get_warehouse_czce(query: web::Query<RankTableQuery>) -> Result<HttpResponse> {
    match futures_warehouse_receipt_czce(&query.date).await {
        ...
        Err(e) => Ok(HttpResponse::InternalServerError().json(ApiResponse::<
            Vec<CzceWarehouseReceiptResponse>,
        >::error(e.to_string()))),
    }
}
...
#[cfg_attr(feature = "swagger", utoipa::path(
    get,
    path = "/futures/warehouse/dce",
    tag = "futures-warehouse",
    params(RankTableQuery),
    responses(
        (status = 200, description = "成功获取仓单日报", body = ApiResponse<Vec<DceWarehouseReceipt>>),
        (status = 500, description = "服务器错误")
    )
))]
...
#[cfg_attr(feature = "swagger", utoipa::path(
    get,
    path = "/futures/warehouse/shfe",
    tag = "futures-warehouse",
    params(RankTableQuery),
    responses(
        (status = 200, description = "成功获取仓单日报", body = ApiResponse<Vec<ShfeWarehouseReceiptResponse>>),
        (status = 500, description = "服务器错误")
    )
))]
pub async fn get_warehouse_shfe(query: web::Query<RankTableQuery>) -> Result<HttpResponse> {
    match futures_shfe_warehouse_receipt(&query.date).await {
        Ok(data) => Ok(HttpResponse::Ok().json(ApiResponse::success(data))),
        Err(e) => Ok(HttpResponse::InternalServerError().json(ApiResponse::<
            Vec<ShfeWarehouseReceiptResponse>,
        >::error(e.to_string()))),
    }
}
...
#[cfg_attr(feature = "swagger", utoipa::path(
    get,
    path = "/futures/warehouse/gfex",
    tag = "futures-warehouse",
    params(RankTableQuery),
    responses(
        (status = 200, description = "成功获取仓单日报", body = ApiResponse<Vec<GfexWarehouseReceiptResponse>>),
        (status = 500, description = "服务器错误")
    )
))]
pub async fn get_warehouse_gfex(query: web::Query<RankTableQuery>) -> Result<HttpResponse> {
    match futures_gfex_warehouse_receipt(&query.date).await {
        Ok(data) => Ok(HttpResponse::Ok().json(ApiResponse::success(data))),
        Err(e) => Ok(HttpResponse::InternalServerError().json(ApiResponse::<
            Vec<GfexWarehouseReceiptResponse>,
        >::error(e.to_string()))),
    }
}
...
#[cfg_attr(feature = "swagger", utoipa::path(
    get,
    path = "/futures/{symbol}/history",
    tag = "futures",
    params(
        ("symbol" = String, Path, description = "合约代码"),
        FuturesQuery
    ),
    responses(
        (status = 200, description = "成功获取历史K线数据", body = ApiResponse<Vec<FuturesHistoryData>>),
        (status = 500, description = "服务器错误")
    )
))]
...
#[cfg_attr(feature = "swagger", utoipa::path(
    ...
    params(FuturesQuery),
    responses(
        (status = 200, description = "成功获取期货列表", body = ApiResponse<Vec<FuturesInfo>>),
        (status = 500, description = "服务器错误")
    )
))]
...
#[cfg_attr(feature = "swagger", utoipa::path(
    get,
    path = "/futures/exchanges",
    tag = "futures",
    responses(
        (status = 200, description = "成功获取交易所列表")
    )
))]
...
#[cfg_attr(feature = "swagger", utoipa::path(
    ...
    responses(
        (status = 200, description = "成功获取多个合约数据", body = ApiResponse<Vec<FuturesInfo>>),
...
#[cfg_attr(feature = "swagger", utoipa::path(
    ...
    tag = "futures",
    responses(
        (status = 200, description = "成功获取品种映射表", body = ApiResponse<Vec<FuturesSymbolMark>>),
        (status = 500, description = "服务器错误")
    )
))]
...
#[cfg_attr(feature = "swagger", utoipa::path(
    get,
    path = "/futures/symbols/{exchange}",
    tag = "futures",
    params(
        ("exchange" = String, Path, description = "交易所代码")
    ),
    responses(
        (status = 200, description = "成功获取指定交易所品种列表", body = ApiResponse<Vec<FuturesSymbolMark>>),
        (status = 500, description = "服务器错误")
    )
))]
...
#[cfg_attr(feature = "swagger", utoipa::path(
    get,
    path = "/futures/main/{exchange}",
    tag = "futures-main",
    params(
        ("exchange" = String, Path, description = "交易所代码")
    ),
    responses(
        (status = 200, description = "成功获取主力合约列表"),
        (status = 500, description = "服务器错误")
    )
))]
...
#[cfg_attr(feature = "swagger", utoipa::path(
    get,
    path = "/futures/{symbol}/detail",
    tag = "futures",
    params(
        ("symbol" = String, Path, description = "合约代码")
    ),
    responses(
        (status = 200, description = "成功获取合约详情", body = ApiResponse<FuturesContractDetail>),
        (status = 500, description = "服务器错误")
    )
))]
...
pub async fn get_spot_price(query: web::Query<FuturesSpotPriceQuery>) -> Result<HttpResponse> {
    let symbols: Option<Vec<&str>> = query
        .symbols
        .as_ref()
...
pub async fn get_spot_price_previous(
    query: web::Query<FuturesSpotPricePreviousQuery>,
) -> Result<HttpResponse> {
...
pub async fn get_spot_price_daily(
    query: web::Query<FuturesSpotPriceDailyQuery>,
) -> Result<HttpResponse> {
    let symbols: Option<Vec<&str>> = query
        .symbols
        .as_ref()
...
pub fn config(cfg: &mut web::ServiceConfig) {
    cfg.service(
        web::scope("/futures")
            ...
            .route(
                "/inventory99/symbols",
                web::get().to(get_inventory99_symbols),
            )
            // 现货价格及基差
            .route("/spot_price", web::get().to(get_spot_price))
            .route(
                "/spot_price_previous",
                web::get().to(get_spot_price_previous),
            )
            ...
            .route(
                "/foreign/{symbol}/history",
                web::get().to(get_foreign_history),
            )
            .route(
                "/foreign/{symbol}/detail",
                web::get().to(get_foreign_detail),
            )
            // 品种实时数据
