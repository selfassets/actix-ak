...
use crate::models::{
    CzceWarehouseReceipt, CzceWarehouseReceiptResponse, DceWarehouseReceipt, ForeignFuturesDetail,
    ForeignFuturesDetailItem, ForeignFuturesHistData, ForeignFuturesSymbol, Futures99Symbol,
    FuturesCommInfo, FuturesContractDetail, FuturesExchange, FuturesFeesInfo, FuturesHistoryData,
    FuturesHoldPosition, FuturesInfo, FuturesInventory99, FuturesMainContract,
    FuturesMainDailyData, FuturesQuery, FuturesRule, FuturesSpotPrice, FuturesSpotPricePrevious,
    FuturesSymbolMark, GfexWarehouseReceipt, GfexWarehouseReceiptResponse, PositionRankData,
    RankSum, RankTableResponse, ShfeWarehouseReceipt, ShfeWarehouseReceiptResponse,
    SinaHoldPosition,
};
...
/// 新浪期货品种映射 JS 文件
const SINA_FUTURES_SYMBOL_URL: &str =
...
/// 九期网期货手续费API
const QIHUO_COMM_URL: &str = "https://www.9qihuo.com/qihuoshouxufei";
...
pub async fn get_futures_comm_info(exchange: Option<&str>) -> Result<Vec<FuturesCommInfo>> {
    ...
    if let Some(table) = document.select(&table_selector).next() {
        for row in table.select(&tr_selector) {
            ...
            // 解析保证金
            let margin_buy = cells
                .get(3)
                .and_then(|s| s.trim_end_matches('%').parse::<f64>().ok());
            let margin_sell = cells
                .get(4)
                .and_then(|s| s.trim_end_matches('%').parse::<f64>().ok());
            let margin_per_lot = cells.get(5).and_then(|s| {
                s.trim_end_matches('元')
                    .replace(",", "")
                    .parse::<f64>()
                    .ok()
            });
            ...
            let (fee_open_ratio, fee_open_yuan) =
                cells.get(6).map(|s| parse_fee(s)).unwrap_or((None, None));
            let (fee_close_yesterday_ratio, fee_close_yesterday_yuan) =
                cells.get(7).map(|s| parse_fee(s)).unwrap_or((None, None));
            let (fee_close_today_ratio, fee_close_today_yuan) =
                cells.get(8).map(|s| parse_fee(s)).unwrap_or((None, None));

            // 解析其他字段
            let profit_per_tick = cells
                .get(9)
                .and_then(|s| s.replace(",", "").parse::<f64>().ok());
...
pub async fn get_futures_spot_price(
    ...
    for row in rows {
        ...
        let dominant_contract_raw = cells
            .get(7)
            .map(|s| s.replace('\u{a0}', ""))
            .unwrap_or_default();
...
fn parse_comm_info_html(html: &str, exchange_filter: Option<&str>) -> Result<Vec<FuturesCommInfo>> {
    ...
    for row_cap in row_re.captures_iter(table_content) {
        ...
        let (fee_open_ratio, fee_open_yuan) =
            parse_fee(cells.get(6).map(|s| s.as_str()).unwrap_or(""));
        let (fee_close_yesterday_ratio, fee_close_yesterday_yuan) =
            parse_fee(cells.get(7).map(|s| s.as_str()).unwrap_or(""));
        let (fee_close_today_ratio, fee_close_today_yuan) =
            parse_fee(cells.get(8).map(|s| s.as_str()).unwrap_or(""));
        ...
        all_data.push(FuturesCommInfo {
            ...
            margin_sell: cells
                .get(4)
                .and_then(|s| s.trim_end_matches('%').parse::<f64>().ok()),
            margin_per_lot: cells
                .get(5)
                .and_then(|s| s.trim_end_matches('元').parse::<f64>().ok()),
            fee_open_ratio,
            fee_open_yuan,
            fee_close_yesterday_ratio,
            fee_close_yesterday_yuan,
...
pub async fn get_futures_hold_pos_sina(
    ...
    let url = format!(
        "{}?t_breed={}&t_date={}",
        SINA_HOLD_POS_API, contract, formatted_date
    );
...
pub async fn futures_shfe_warehouse_receipt(
    ...
    // 按品种分组
    let mut grouped: std::collections::HashMap<String, Vec<ShfeWarehouseReceipt>> =
...
mod tests {
    ...
    fn test_is_cffex_symbol() {
        ...
        let cffex_symbols = vec![
            "IF2401", "IC2401", "IH2401", "T2406", "TF2406", "TS2406", "IM2401", "TL2406",
        ];
        for symbol in &cffex_symbols {
            assert!(
                service.is_cffex_symbol(symbol),
                "{} 应该是中金所合约",
                symbol
            );
        }
        ...
        match get_futures_hold_pos_sina("volume", "RB2510", "20250107").await {
            Ok(positions) => {
                ...
                for p in positions.iter().take(10) {
                    println!(
                        "  {:<6} {:<20} {:>12} {:>12}",
                        p.rank, p.company, p.value, p.change
                    );
                }
            }
            Err(e) => {
            ...
            Ok(data) => {
                ...
                for d in data.iter().take(10) {
                    ...
                    let fee_today = d
                        ...
                        .map(|v| format!("{}元", v))
                        .or_else(|| {
                            d.fee_close_today_ratio
                                .map(|v| format!("{:.4}‱", v * 10000.0))
                        })
                        .unwrap_or("-".to_string());
                    println!(
                        "  {:<12} {:<10} {:<8} {:>8.0} {:>8.1} {:>10} {:>10}",
                        d.exchange,
                        d.contract_name,
                        d.contract_code,
    ...
    async fn test_futures_spot_price() {
        ...
        match get_futures_spot_price("20240430", None).await {
            Ok(data) => {
                ...
                for d in data.iter().take(15) {
                    println!(
                        "  {:<8} {:>10.2} {:>12} {:>10.2} {:>12} {:>10.2} {:>10.2}",
                        d.symbol,
                        d.spot_price,
                        d.near_contract,
                        d.near_contract_price,
                        d.dominant_contract,
                        d.dominant_contract_price,
                        d.dom_basis
                    );
                }
            }
        ...
        match get_futures_spot_price("20240430", Some(vec!["RB", "CU", "AU"])).await {
            Ok(data) => {
                ...
                for d in &data {
                    println!(
                        "    【{}】现货:{:.2} 主力:{} 价格:{:.2} 基差:{:.2} 基差率:{:.2}%",
                        d.symbol,
                        d.spot_price,
                        d.dominant_contract,
                        d.dominant_contract_price,
                        d.dom_basis,
                        d.dom_basis_rate * 100.0
                    );
                }
            }
            Err(e) => {
            ...
            Ok(data) => {
                ...
                for d in data.iter().take(10) {
                    println!(
                        ...
                        d.variety,
                        d.vol_top5,
                        d.long_open_interest_top5,
                        d.short_open_interest_top5,
                        d.date
                    );
                }
            }
    ...
    async fn test_get_rank_sum_daily() {
        ...
        // 测试获取单日数据
        println!("\n  1. 测试获取单日数据（20250107）:");
        match get_rank_sum_daily(
            "20250107",
            "20250107",
            Some(vec!["RB".to_string(), "CU".to_string()]),
        )
        .await
        {
            Ok(data) => {
                ...
                for d in data.iter().take(10) {
                    println!(
                        ...
                        d.variety,
                        d.vol_top10,
                        d.long_open_interest_top10,
                        d.short_open_interest_top10
                    );
                }
            }
            Err(e) => {
        ...
        match get_rank_sum_daily("20250106", "20250107", Some(vec!["IF".to_string()])).await {
            Ok(data) => {
                ...
                for d in data.iter().take(10) {
                    println!(
                        "    {} {} - 成交量Top20:{} 多单Top20:{} 空单Top20:{}",
                        d.date,
                        d.symbol,
                        d.vol_top20,
                        d.long_open_interest_top20,
                        d.short_open_interest_top20
                    );
                }
            }
            Err(e) => {
            ...
            Ok(data) => {
                ...
                for item in data.iter().take(5) {
                    ...
                    for row in item.data.iter().take(3) {
                        println!(
                            "      {} - 仓单:{:?} 有效预报:{:?} 增减:{:?}",
                            row.warehouse, row.warehouse_receipt, row.valid_forecast, row.change
                        );
                    }
                }
            }
    ...
    async fn test_futures_warehouse_receipt_dce() {
        ...
        match futures_warehouse_receipt_dce("20240701").await {
            Ok(data) => {
                println!("  ✅ 获取成功！共 {} 条数据", data.len());
                // 统计品种数量
                let mut varieties: std::collections::HashSet<&str> =
